...
if __name__ == '__main__':
    ...
    with h5py.File(fileName,'a') as f:
        print(sourceDataSetName)
        statesSource = openDataSet(sourceDataSetName)
        l = len(statesSource)
        shape = (l, 4, 8, 8)
        sourceBuffer = statesSource[:]
        ...
        targetBuffer = np.array(np.zeros(shape), dtype='bool_')
        print("Creating target buffer finished")

        for vecState in sourceBuffer:
            targetBuffer[i2] = vecSt2fullSt(vecState, nPi, nPa, nWPa)
            if i2%1000:
                t2 = time()
